...
async function main() {
    ...
    const allowance = await programAddress([account1.toBuffer(), delegate.publicKey.toBuffer()], securityTokenPK)
    const tx2 = new anchor.web3.Transaction()
    tx2.add(
        securityToken.instruction.delegateApprove(
            new anchor.BN(0),
            true,
            {
                accounts: {
                    allowance: new PublicKey(allowance.pubkey),
                    owner: owner,
    ...
    console.log(await provider.sendAndConfirm(tx2))
}
